...
def extract_vein_features(binary_image):
    ...
    thin_contour = cv2.ximgproc.thinning(binary_image)
    # cv2.imshow("Skel", thin_contour)
    # cv2.waitKey(0)
    # cv2.destroyAllWindows()
    vein_density = np.count_nonzero(thin_contour) / thin_contour.size

    yx_coords = np.column_stack(np.where(thin_contour > 0))
...
def visualize_contour(image, contour):
    ...
    cv2.imshow("Contour Visualization", image_copy)
    cv2.waitKey(0)
    cv2.destroyAllWindows()

if __name__ == "__main__":
    image_path = "ClassificationModel/Image_Output/29/1.jpg"  # Replace with your image path
    image = cv2.imread(image_path)
    binary = preprocess_image(image)
    tight, loose = compute_contour(binary)
    transparent = np.zeros((image.shape[0], image.shape[1], 4), dtype=np.uint8)
    cv2.imshow("Binary", binary)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
    visualize_contour(transparent, tight)
    visualize_contour(transparent, loose)
